...
We consider a one-dimensional elastic interface described by a displacement field (u_i) defined on a periodic lattice of size (L). The local slope is
...
To obtain the equilibrium equations, vary the Hamiltonian with respect to (u_i).
...
The equilibrium configuration is therefore uniquely determined by a single scalar parameter (C).
...
The original (L)-dimensional minimization problem is thus reduced to a one-dimensional root-finding problem for the integration constant (C).
